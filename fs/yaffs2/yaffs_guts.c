 ...
 */
static u32 yaffs_del_dir_contents(struct yaffs_obj *dir)
{
	u32 remove_count = 0;
	struct yaffs_obj *obj;
	struct list_head *lh;
	...
	list_for_each_safe(lh, n, &dir->variant.dir_variant.children) {
		...
		if (obj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY)
			remove_count += yaffs_del_dir_contents(obj);
		yaffs_trace(YAFFS_TRACE_SCAN,
...
{
	dev->remove_lost_n_found = yaffs_del_dir_contents(dev->lost_n_found);
}
